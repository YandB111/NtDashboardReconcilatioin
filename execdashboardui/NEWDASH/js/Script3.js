...
$(function() {
	$("#key_op").daterangepicker({
	  singleDatePicker: true,
	  showDropdowns: true,
	}, 
	
	function(start) {
		$('#key_op').html(start.format('DD-MMM-YYYY'))
	 keyop_date = start._d;
	 keyop_date = new Date(keyop_date).toUTCString();
	 keyop_date = keyop_date.split(' ').slice(1, 4).join('-');

	 ntable.innerHTML = '';
	 Key_Operation();
		//console.log(keyop_date);
	});
  });
...
function main() {
	...
	var Dashboard =
		$.ajax({
			url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/Graph_validation',
			dataType: 'json',
...
function lineChart() {
	...
	$.ajax({
		url: "https://192.168.167.5:8743/Dashboard/dashboard/api/monthlyRevenue?Start_date=" + strtDate + "&End_date=" + endDate + "&User=" + usertype + "&Service=" + selectservice,
...
// document.getElementById("pie").innerHTML= "% of Total Revenue " +pie_mnth+" "+pie_year;
	...
	$.ajax({
		url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/totalRevenue?year=' + pie_year + '&month=' + pie_mnth,
		dataType: 'json',
		//var a = (jsonData.frodcaseClose);
		success: function (jsonData) {
...
function KRF_table() {
	$.ajax({
		url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/comaprisonRevenue?Start_Month='+krf_strt+'&Comparison_Month='+krf_end,
		dataType: 'json',
		success: function (jsonData) {
			...
			$.each(demodata, function (index, data) {
				...
				tabledata += '<tbody><tr><td>' + data.key_Revenue_Figures + '</td><td style="text-align:right">' + data.lastMonthRev + '</td><td style="text-align:right">' + data.currntmonthRevenue + '</td><td style="text-align:right"> ' + data.w_r_t_last_month + '<img src="https://192.168.167.5:8743/cs-execdashboard/portlets/ctva/execdashboardui/NEWDASH/images/' + imgArray[index] + '"></img></td></tr></tbody>';
			});
...
function TopX_Roaming() {
	...
	$.ajax({
		url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/TopxRoamingPartner?year=' + top10_year + '&month=' + top10_mnth,
		dataType: 'json',
		success: function (jsonData) {
...
function TopX_Recharge() {
	...
	//document.getElementById("Top4").innerHTML= top4_mnth+" "+top4_year;
	$.ajax({
		url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/TopxRechargeVoucher?year=' + top4_year + '&month=' + top4_mnth,
...
function Key_Operation() {
	$.ajax({
		url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/KeyOperational?ShowDate='+keyop_date,
		dataType: 'json',
		success: function (jsonData) {
			function unpack(data, key) {
...
function Key_Reconcillation() {
	$.ajax({
		url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/Key_Reconciliation?year=2020&month=jul',
		dataType: 'json',
		success: function (jsonData) {
			function unpack(data, key) {
...
function Key_Cases() {
	$.ajax({
		url: 'https://192.168.167.5:8743/Dashboard/dashboard/api/Alarm_KPIs',
		dataType: 'json',
		success: function (jsonData) {
			var a = (jsonData.frodcaseClose);
			function unpack(data, key) {
