...
function Table_1() {
    $.ajax({
        url: url+'Roaming_out_Rev?year='+Cy+'&month='+Cm+'&currency=thousand',
        dataType: 'json',
...
function Table_2() {
    $.ajax({
        url: url+'Roaming_out_EventRev?year='+Cy+'&month='+Cm+'&currency=thousand',
        dataType: 'json',
        success: function (jsonData) {
            function unpack(data, key) {
                return data.roamingOutEventByRevList.map(function (row) {
                    return row[key];
                });
            }
            var Month = jsonData.month;
            var demodata = jsonData.roamingOutEventByRevList;
            var day = '';
            var onnet = '';
            var offnet = '';
            var national = '';
