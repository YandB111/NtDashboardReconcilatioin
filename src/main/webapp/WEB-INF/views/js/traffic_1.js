...
var chartType = '';
var unit = 'Million';
var url = 'https://10.26.194.52:8743/Dashboard/dashboard/traffic/';
...
function Table_1() {
    $.ajax({
        url: url + 'trafficRevenueDayByDay?year=' + chartYear + '&month=' + chartMonth + '&currency=' + unit,
        dataType: 'json',
        success: function (jsonData) {
            ...
            var field = new Array(count);
            var position = new Array(count);
            var pos=1;
            for (var i = 0; i < count; i++) {
                field[i]=name[i];
                position[i]=pos;
                pos++;
                // console.log(field[i],position[i]);   // also declare an array of position to match the positions of the fields
            }
            // console.log(name); use if else conditions on fields data

          //  console.log(name[1]);// try to fetch variables from the array of names so as to use in table creation
            var demodata = jsonData.trafficDayByRevList;
            var day = '';
            // var count='';
            var callfrwd = '';
            ...
            $('#ntable').append('<table style="width:0px id="tablevalue_top" class="table table-bordered mb-0">'
                ...
                + '<tr>'
                + '<td><b>' + name[1] + '</b></td>'
                + '<td style="text-align:right">' + total_gprs + gprs + '</td>' //change these!!!!!!!!!!!!!!!!!!!!
                + '</tr>'
                + '<tr>'
                + '<td><b>' + name[2] + '</b></td>'
                + '<td style="text-align:right">' + total_sms + smsrev + '</td>'// look at script2 and refer to key operational table
                + '</tr>'
                + '<tr>'
                + '<td><b>' + name[3] + '</b></td>'
            ...
            var tracepie = {
                type: 'pie',
                values: [total_voice, total_sms, total_callfrwd, total_gprs, total_ussd, total_videotele, total_fax],
                labels: ['Voice', 'SMS', 'Call Forward', 'GPRS', 'USSD', 'Video Telephony', 'FAX'],
                textinfo: 'percent',
                textposition: "inside",
                //textposition: "inside",
            ...
            var data = [trace3, trace2, trace6, trace4, trace1, trace5, trace8];
            var data1 = [trace7];
            var datapie = [tracepie];

            var layout = {
                title: 'Day-On-Day Event Wise Growth Analysis And Traffic Statistics',
                autosize: true,
                width: 800,
                height: 306,
                dragmode: "pan",
            ...
            var layout1 = {
                autosize: true,
                title: 'Day On Day Growth Analysis(%)',
                width: 1300,
                height: 250,
                dragmode: "pan",
                barmode: "group",
            ...
            var layoutpie = {
                showlegend: true,
                title: 'Event Wise Monthly Traffic Bifurcation',
...
function Table_2() {
    $.ajax({
        url: url + 'trafficRevenueGraph?year=' + chartYear + '&month=' + chartMonth + '&currency=' + unit,
        dataType: 'json',
        success: function (jsonData) {
            function unpack(data, key) {
            ...
            var data1 = [trace7]
            var layout_bar = {
                title: 'Day-On-Day Event Category Wise Growth Analysis And Traffic Statistics',
                autosize: true,
                width: 800,
                height: 306,
                dragmode: "pan",
            ...
            var layout_pie = {
                showlegend: true,
                title: 'Category Wise Monthly Traffic Bifurcation',
                margin: { "t": 30, "b": 0, "l": 40, "r": 0 },
                height: 306,
                width: 400,
                displayModeBar: false,
...
function Table_3() {
    $.ajax({
        url: url + 'trafficRevenueCategory?year=' + chartYear + '&month=' + chartMonth + '&currency=' + unit,
        dataType: 'json',
        success: function (jsonData) {
            function unpack(data, key) {
            ...
            var layout_bar = {
                title: 'Day-On-Day Status Wise Growth Analysis And Traffic Statistics',
                autosize: true,
                width: 800,
                height: 306,
                dragmode: "pan",
            ...
            var layout_pie = {
                showlegend: true,
                title: 'Status Wise Monthly Bifurcation',
                margin: { "t": 30, "b": 0, "l": 40, "r": 0 },
                height: 306,
                width: 400,
                displayModeBar: false,
